...
class ModelConfig(BaseModel, frozen=True):
    ...
    tgt_vocab_size: int = Field(default=1028, gt=0) # Increased slightly to accommodate BOS/PAD/EOS > 1024
...
# Removed the deprecated TrainingConfig class
...
class DiaConfig(BaseModel, frozen=True):
    ...
    Attributes:
        ...
        architectures: list[str] = Field(default_factory=lambda: ["DiaModel"])
    """
    ...
    # HF Hub compatibility fields (optional but good practice)
    model_type: str = Field(default="dia")
    architectures: list[str] = Field(default_factory=lambda: ["DiaModel"])
    ...
    def save(self, path: str | Path) -> None:
        ...
        Args:
            path: The target file path (string or Path object) to save the configuration.
        """
        save_path = Path(path)
        if save_path.suffix != ".json":
             save_path = save_path.with_suffix(".json") # Ensure .json extension

        os.makedirs(save_path.parent, exist_ok=True)
        # No need to exclude 'training' anymore
        config_json = self.model_dump_json(indent=2)
        with open(save_path, "w", encoding="utf-8") as f:
            f.write(config_json)

    @classmethod
    def load(cls, path: str | Path) -> "DiaConfig | None":
        """Load and validate a Dia configuration from a JSON file.

        Args:
            path: The path (string or Path object) to the configuration file.
        ...
        if not load_path.exists() or not load_path.is_file():
             print(f"Config file not found at: {load_path}")
             return None
        if load_path.suffix != ".json":
             print(f"Warning: Config file does not have .json extension: {load_path}")
        ...
        try:
            ...
            return None
             ...
             print(f"Configuration validation error loading {load_path}: {e}")
             raise
        except Exception as e:
             print(f"Error loading or parsing config file {load_path}: {e}")
             raise